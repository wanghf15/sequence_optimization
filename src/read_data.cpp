...
namespace sequence_optimization {
    ...
    vector<vector<double >> JsonReader::read_by_tracklets_from_file(string filename) {
//        json tracklets;
...
//        } // end of frames
    ...
    map<int, vector<vector<double >>> JsonReader::read_by_frames_from_path(string path) {
        vector<string> json_path_list = globVector(path + "*.json");
        ...
        for (int clipidx = 0; clipidx < json_path_list.size(); clipidx++) {
            ...
            map<int, vector<vector<double>>> id_to_car;

            for (auto it = frames.begin(); it != frames.end(); ++it) {
                json cars = it.value();
                cars = cars["cars"];
                for (json car : cars) {
//                    cout << car["car_type"] << endl;
                    int track_id = car["track_id"];
                    vector<double> cuboid_2d = car["cuboid_2d"];
                    vector<double> cuboid_3d = car["cuboid_3d"];
